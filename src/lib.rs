...
use pb::starknet::v1::*;
use crate::abi::coursecontract_contract::Event as CoursecontractEvent;
...
fn map_coursecontract_events(transactions: Transactions) -> Result<Events, substreams::errors::Error> {
    ...
    for transaction in transactions.transactions_with_receipt {
        ...
        for event in data_events {
            ...
            if event_from_address != "03f90884eca9e36599467b8178fa9ee5950df6fd22c06102a8effc58bb8945db" {
                continue;
            }
            ...
            if let coursecontract_event = CoursecontractEvent::try_from(emitted_event).unwrap() {
                let event_json = serde_json::to_string(&coursecontract_event).unwrap();
                let event = Event {
